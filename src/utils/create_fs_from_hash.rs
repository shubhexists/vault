...
pub fn create_fs(commit_hash: &String, path: &String, branch: &String) -> io::Result<()> {
    ...
    let _ = navigate_through_root_dir_hash(&root_dir_hash, branch);
    Ok(())
}

fn navigate_through_root_dir_hash(root_dir_hash: &String, branch: &String) -> io::Result<()> {
    ...
    let file: File = File::open(hash_file_path)?;
    let commit_string: String = decompress_zlib(file).unwrap();
    let tree_object = Tree::get_tree_from_content(&commit_string);
    println!("{:?}", tree_object);
    Ok(())
}
